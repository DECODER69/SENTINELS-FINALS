...
print('')
url = 'http://192.168.18.107:4000/data'
...
#         check =  {"date_time": str(datetime.now().strftime("%B %d, %Y - ")+ str(datetime.now().strftime("%H:%M %S sec"))), "captured": str(imageBase64.decode("utf-8"))}
...
while True:
    ...
    if status_list[-1] == 0 and status_list[-2] == 1:
        ...
        cv2.imwrite("./captured/"+ filename +".jpg", frame)
        print(filename)
        # convertToBinaryData("./captured/"+ filename +".jpg")
        # Get frame
